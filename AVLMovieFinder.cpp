...
AVLMovieFinder::AVLMovieFinder() {
    this->tree = new BalancedBST<Movie>();
}

// Constructor de còpia
AVLMovieFinder::AVLMovieFinder(const AVLMovieFinder& orig) {
    this->tree = new BalancedBST<Movie>(*orig.tree);
}

// Destructor
AVLMovieFinder::~AVLMovieFinder() {
    this->tree->~BalancedBST();
}
...
string AVLMovieFinder::showMovie(int movieID) const {
...
string AVLMovieFinder::findTitleMovie(int movieID) const {
...
float AVLMovieFinder::findRatingMovie(int movieID) const {
...
BalancedBST<Movie>* AVLMovieFinder::getTree() const {
    return tree;
}

// Funció que mostra les Movies de l'arbre per ID en ordre creixent
void AVLMovieFinder::printInorder() const {
...
int AVLMovieFinder::printInorder(const NodeTree<Movie>* p, int count) const {
...
void AVLMovieFinder::parseMovieData(string data, int& movieID, string& title, float& rating) const {
